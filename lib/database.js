...
let messengerResponses = [];

database.ref('messenger/responses').on('value', data => {
    messengerResponses = data.val().sort((a, b) => a.priority - b.priority);
});
 ...
 *
 *  @return A Promise which will resolve to an array of messenger patterns and
 *      responses from the Firebase reference.
 **/
